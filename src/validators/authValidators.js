...
export const authValidators = {
  ...
  setupTwoFactor: Joi.object({
    ...
    backupCodes: Joi.array()
      .items(Joi.string().length(8).pattern(/^[A-Z0-9]{8}$/))
      .length(10)
    .optional()
    .messages({
        'array.length': 'Must provide exactly 10 backup codes',
        'string.length': 'Each backup code must be 8 characters long',
        'string.pattern.base': 'Backup codes must contain only uppercase letters and numbers',
      }),
  }),
  ...
  bulkUserOperation: Joi.object({
    ...
    operation: Joi.string()
      .valid('activate', 'deactivate', 'suspend', 'delete', 'verify', 'unverify')
      .required()
      .messages({
